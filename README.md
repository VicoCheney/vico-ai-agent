...
> 默认配置 `auto_approve: ["low"]`，即读取和搜索自动执行，`write` / `edit` 需要确认。如需更激进的自动修改，可改为 `["low", "medium"]`。

> `read` / `search` 会拒绝读取常见敏感文件，例如 `.env`、私钥和证书文件；请使用脱敏示例文件排查配置问题。
...
```jsonc
{
  ...
  "tools": {
    "auto_approve": ["low"],            // 自动批准的风险级别（low=read/search）
